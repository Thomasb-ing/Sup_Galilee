...
int main( int nargs, char* argv[] )
{
    ...
    // on va utiliser le proc 0 pour afficher et le proc 1 pour calculer
    
    int job = 0 ;
    

    // Initaialisation des variables systemes utiles pour les 
    char const* filename;
        if (nargs==1)
        {
    ...
    if(rank == 0)
    {
        ...
        while (myScreen.isOpen())
        {
            ...
            MPI_Recv();
            // Puis comme il a plus rien à faire, on lui renvoie du travail !

            // affichage des points que le Slave (oups stagiaire) nous a gentillement calculé .
            ...
            // Slave AU TRAVAIL !
            MPI_Send(&job,1,MPI_INT64_T,1,101,comm);
            // Après la pause café, on souhaite récuperer le travail de notre stagiaire !
        ...
        {
            MPI_Status status;
            // HELLO IM PROC 1 MY ROLE IS TO CALCULATE BECAUSE IM A SLAVEEE !
            // I need to receive order to begin from my master
            MPI_Recv(&job,1,MPI_INT64_T,0,101,comm,&status);
                        if (animate | advance)
            {            
                if (isMobile)
                {
                ...
                else
                {
                 ...
                 //on envoie isMobile, dt, grid, cloud, et vortices si isMobile = True
                // calcul des nuages de points (on va faire ca sur un processeur de rank 0)
            }
        }
    MPI_Finalize();
    return EXIT_SUCCESS;
